...
<template>
  <div class="boxCountry"> 
  	<input autocomplete="off" type="text" placeholder="请选择目的国家/区域，例如：美国、英国、德国" @focus="select($event)" @input="getVal($event)" id="vueCountry">	
	<div class="box-city">
		<div class="box-top">
			<ul>
		...
		<div class="box-bottom">
			<div class="content" v-for="(country,i) in list" v-if="country.data != ''">
				...
				<ul>				
					<li v-for="(name,j) in country.data"><span @click="chioceCountry($event)" :class="[country.inputVal.includes(name) ? 'active' : '']">{{name}}</span></li>
				</ul>
			</div>			
		</div>		
	</div>
	<div class="box-ul">
		<ul>
			<li v-for="(name,i) in ulCountry">{{name}}</li>				
		</ul>
	</div>	
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    select(e){	
		...
		//获取原有输入框的值
		const inputVal = document.getElementById('vueCountry').value;
		let arr;
		if(inputVal.trim() == ""){
			 arr = [];
		}else{
	...
	chioceCountry(e){
		const val = e.target.innerHTML;
		//获取原有输入框的值
		let inputVal = document.getElementById('vueCountry').value;
		if(inputVal.trim() == ""){
			e.target.classList.add('active');
			document.getElementById('vueCountry').value = val;
		}else{
			...
			if(setRepeat == true){
				...
				document.getElementById('vueCountry').value = inputVal;
				e.target.classList.remove('active');
			}else{
				e.target.classList.add('active');
				document.getElementById('vueCountry').value = inputVal+'、'+val;
			}			
		}
